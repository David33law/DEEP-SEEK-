...
The DEEP-SEEK- repository is an external architecture-discovery laboratory. Its sole Observatory objective is to discover, engineer, falsify and justify the strongest technically constructible whole-system architecture for a national canonical information infrastructure of the Greek legal order.

The target repository under study is evidence, not the research harness. The harness must not lower the target to fit its current evaluator, implementation language, budget, existing architecture, convenient database, first successful candidate or first compelling model answer.

The Observatory itself must make legally material change observable rather than silently lossy; reconstruct legal/effective and system-knowledge time; preserve primary evidence and historical states; link jurisprudence to exact authority versions; keep doctrine epistemically separate; carry independently checkable provenance; expose unknown/conflict states; rebuild deterministically; and publish one canonical truth through human, API, linked-data, ELI, public-sector and AI projections.

## 2. Why this standard exists

A national legal information infrastructure can cause downstream humans, institutions and AI systems to act on whatever it represents as law. A merely adequate architecture therefore fails the mission. The research protocol must search for architectural weaknesses before they become institutional weaknesses: silent source loss, false identity, temporal collapse, untraceable legal effect, competing truth seats, non-reproducible projections, brittle governance, unrecoverable state or an architecture that cannot evolve without reopening its trusted core.

## 3. Engineering bar

The required bar is first-principles, frontier engineering: question inherited assumptions, maximize evidence per trusted mechanism, keep the trusted kernel as small and independently checkable as the problem permits, make failures observable, and spend additional search/implementation effort while it continues to produce evidence-supported architectural improvement.

References to an "xAI/SpaceX-class" or "Elon-Musk-level" bar are shorthand for this engineering standard. They are NOT evidence about what Elon Musk personally would choose or say. The final result may state that it is designed to satisfy a first-principles frontier-engineering standard only through the evidence listed below.

## 4. No first-answer privilege

No individual model response can be a final architecture. Every surviving architecture must emerge from a search forest containing independent lineages, structurally constrained anti-attractor lineages, explicit rejected alternatives, executable implementations, adversarial attacks, recombinations and simplification attempts.

A model running at maximum reasoning effort is still one search trajectory. The protocol must buy breadth with independent trajectories and depth with expansion, formalization, implementation, falsification and revision.

## 5. Architecture genome

Architecture-family diversity is measured by load-bearing structure, never by names. Every candidate must declare at least these genome axes:

1. canonical authority seat;
2. evidence primitive;
3. legal-object identity model;
4. canonical state-derivation model;
5. temporal representation;
6. normative-effect representation;
...
The purpose is not to force novelty for novelty's sake. If independent searches converge, the protocol must distinguish genuine convergence from prompt-induced anchoring by attacking the shared assumptions and measuring the resulting alternatives.
...
Every finalist must be converted into an explicit architecture formalization containing:

- invariant identifiers and failure conditions;
- authority/write boundaries;
- state and transition semantics;
- temporal and effect semantics;
- proof obligations;
- recovery obligations;
- publication invariants;
- governance/evolution invariants;
- forbidden shortcuts;
- claims that cannot yet be tested.
...
- a candidate passes the current tests;
- several model responses converge;
