#!/usr/bin/env python3
"""Authoritative zero-cost E2E proof for Observatory protocol v4.

The proof clones the exact committed runner, enables only the signed reduced proof workload, performs
the owner ceremony and all local evaluator calibrations, drives the real shared state machine through
every owner gate against the complete local mock provider, verifies all semantic/durable/distributed/
scale/formal/interoperability/cross-model/prior-art/novelty evidence, and proves every terminal
condition fail-closed. It never contacts the real DeepSeek endpoint.
"""
...
RUN_ID = "OBS-PROTOCOL-PROOF-V4-0001"
...
def short_workspace():
    if os.name != "nt":
        return tempfile.mkdtemp(prefix="observatory-protocol-proof-")
    ...
    return tempfile.mkdtemp(prefix="v4-", dir=parent)


def clone_exact(destination):
    watched = ["profiles/national-observatory", "run_observatory.py",
               "setup_observatory.py", "executable-orchestrator/lawmax21",
               "private-evaluator/evaluator", "benchmark",
               "executable-orchestrator/tools"]
    status = run(git_argv("-C", ROOT, "status", "--porcelain",
                          "--untracked-files=all", "--", *watched))
    if status.returncode != 0 or (status.stdout or "").strip():
        raise RuntimeError("source checkout has uncommitted protocol drift:\n"
    ...
    clone = run(git_argv("clone", "--no-hardlinks", "--quiet", ROOT, destination))
    if clone.returncode != 0:
        raise RuntimeError("disposable clone failed: " + clone.stderr)
    checkout = run(git_argv("-C", destination, "checkout", "--quiet", "--detach", head))
...
def drive_launch(repo, target, cp1, prior, runtime, owner_key, endpoint, env):
    ...
    signer = os.path.join(repo, "executable-orchestrator", "tools", "owner_sign.py")
    arguments = ["--launch", "--run-id", RUN_ID, "--endpoint", endpoint,
                 "--model", MODEL, "--backend", "subprocess",
    ...
    try:
        ...
        for index, char in enumerate(stripped):
            ...
            try:
                ...
                best = value if stripped[index + end:].strip() == "" else best
            except Exception:
                continue
        if best is None:
...
def verify_ledger(runtime):
    ...
    if not entries:
        raise RuntimeError("mock provider ledger contains no settled calls")
    for index, entry in enumerate(entries):
        ...
        hit, miss, prompt = (usage.get("prompt_cache_hit_tokens"),
                             usage.get("prompt_cache_miss_tokens"),
                             usage.get("prompt_tokens"))
        if None in (hit, miss, prompt) or int(hit) + int(miss) != int(prompt):
            raise RuntimeError(f"ledger entry {index} lacks exact cache split")
    return {"settled_mock_calls": len(entries),
...
def verify_protocol(repo, runtime, source_head, preflight, launch):
    ...
    if summary.get("final_state") != "COMMITTED" or summary.get("log_verified") is not True:
        raise RuntimeError("run did not finish COMMITTED with a verified signed log")
    ...
    if not conditions or any(value is not True for value in conditions.values()):
        raise RuntimeError("terminal conditions are incomplete: "
                           + ", ".join(key for key, value in conditions.items() if not value))
    missing_new = sorted(EXPECTED_NEW_CONDITIONS - set(conditions))
    if missing_new:
        raise RuntimeError("new protocol conditions absent: " + ", ".join(missing_new))
    supremacy = escalation.get("supremacy") or {}
    if any(supremacy.get(key) is not True for key in EXPECTED_NEW_CONDITIONS):
        raise RuntimeError("new supremacy summary contains a non-passing condition")

    mission = preflight.get("signed_mission") or {}
    if mission.get("protocol_version") != "OBSERVATORY-OMEGA-RESEARCH-PROTOCOL-4":
        raise RuntimeError("signed mission is not protocol v4")
    if mission.get("runner_head") != source_head:
        raise RuntimeError("signed mission is not bound to proof source HEAD")
    for flag in ("cross_model_consistency_required",
    ...
    if missing_files:
        raise RuntimeError("signed protocol census omitted: " + ", ".join(missing_files))
    if preflight.get("proof_mode") is not True:
        raise RuntimeError("zero-cost proof did not run under explicit proof mode")
    ...
    if receipt.get("proof_mode") is not True or receipt.get("provider_calls") != 0:
        raise RuntimeError("specialized calibration receipt has wrong mode/provider count")
    if receipt.get("protocol_bundle_sha256") != mission.get(
            "research_protocol_bundle_sha256"):
        raise RuntimeError("specialized calibration receipt is not bound to signed protocol")
    for label in ("distributed", "scale", "formal", "interoperability"):
        row = (receipt.get("campaigns") or {}).get(label) or {}
        if row.get("status") not in ("PASS", "OK") or row.get("passed") is not True:
    ...
    if audit.get("protocol_version") != mission.get("protocol_version") \
    ...
    if not audit.get("immutable_package_unchanged") \
        ...
        raise RuntimeError("independent audit failed immutable/hidden guarantees")
    audit_conditions = audit.get("supremacy_conditions") or {}
    if audit_conditions != conditions:
        raise RuntimeError("independent audit did not reproduce terminal conditions exactly")
    ...
    if fsum.get("lineages") != 15 or fsum.get("general_lineages") != 6 \
        ...
        raise RuntimeError("search forest is not the required independent 15-lineage forest")
    ...
    if proposals.get("prior_cp2_visible") is not False \
        ...
        raise RuntimeError("independent complete proposal frontier is too small or CP2-contaminated")
    ...
    if built.get("complete_blueprint_handoff") is not True \
        ...
        raise RuntimeError("complete blueprint/formalization implementation handoff failed")

    incumbent = escalation["incumbent"]
    assert_pass(os.path.join(runtime, "reports", f"systems-crown-{incumbent}.json"),
                "durable crown")
    assert_pass(os.path.join(runtime, "reports", f"distributed-crown-{incumbent}.json"),
                "distributed crown")
    assert_pass(os.path.join(runtime, "reports", f"scale-crown-{incumbent}.json"),
                "scale crown")
    cross = assert_pass(os.path.join(runtime, "reports", f"cross-model-crown-{incumbent}.json"),
                        "cross-model crown")
    expected_histories = int((preflight.get("signed_mission") or {}).get(
        "production_workloads", {}).get("cross_model", {}).get("crown_histories", 0))
    proof_histories = int((audit.get("workload_policy") or {}).get(
        "cross_model", {}).get("crown_histories", 0))
    if cross.get("histories") != proof_histories or expected_histories <= proof_histories:
        raise RuntimeError("cross-model crown did not use the signed proof workload boundary")
    ...
    if len(formal_crown) != 2 or len(interop_crown) != 2:
        raise RuntimeError("independent formal/interoperability crown evidence count is wrong")
    formal_reports = [assert_pass(path, "formal crown") for path in formal_crown]
    interop_reports = [assert_pass(path, "interoperability crown") for path in interop_crown]
    ...
    if prior.get("all_sources_assessed") is not True \
        ...
        raise RuntimeError("authoritative prior-art campaign did not close")
    for critic in prior.get("critics") or []:
        for review in (critic.get("report") or {}).get("source_reviews") or []:
            if review.get("disposition") == "SATISFIED":
                ...
                if not verified:
                    raise RuntimeError("SATISFIED prior-art review has no verified evidence")
                for evidence in verified:
                    path = os.path.join(runtime, *evidence["path"].split("/"))
                    if not os.path.isfile(path) or sha256_file(path) != evidence["sha256"]:
                        raise RuntimeError("prior-art verified evidence hash mismatch")
    ...
    for path in novelty_paths[-3:]:
        wave = read_json(path); ledger = wave.get("ledger_wave") or {}
        if ledger.get("dry") is not True or ledger.get("methods_complete") is not True \
                or ledger.get("backlog_count") != 0 or ledger.get("unresolved_count") != 0 \
                or ledger.get("prior_cp2_direct_content_read") is not False:
            raise RuntimeError("final novelty waves are not genuinely dry")
        round_number = re.search(r"r(\d+)\.json$", path).group(1)
        meta = read_json(os.path.join(
            runtime, "architecture", f"meta-search-wave-r{round_number}.json"))
        if len(meta.get("meta_critics") or []) != 2 \
            ...
            raise RuntimeError("meta-search/closure evidence incomplete")
    ...
    if lower.get("closed") is not True or destroyers.get("survived") is not True \
            or case.get("mechanically_supported") is not True \
            or case.get("third_party_endorsement_claimed") is not False:
        raise RuntimeError("lower-bound/destroyer/public supremacy evidence did not close")
    ...
    if reality.get("repository_archaeology_api_calls") != 0 \
        ...
        raise RuntimeError("CP1 reuse / pre-frontier CP2 quarantine failed")
    return {"summary": summary, "audit": audit, "incumbent": incumbent,
            "conditions": sorted(conditions), "search_forest": fsum,
...
def main(argv=None):
    ...
    workspace = short_workspace(); repo = os.path.join(workspace, "r")
    runtime = os.path.join(workspace, "rt"); mock = None
    env = dict(os.environ); env[PROOF_ENV] = "1"
    env["PYTHONDONTWRITEBYTECODE"] = "1"
    result = {"proof": "complete-observatory-protocol-zero-cost-e2e-v4",
              "paid_api_calls": 0, "workspace": workspace, "runtime": runtime,
              "status": "FAIL"}
    try:
        source_head = clone_exact(repo); result["source_head"] = source_head
        setup = py(os.path.join(repo, "setup_observatory.py"), "--run-id", RUN_ID,
                   "--budget-usd", "500", "--tokens", "500000000",
                   "--calls", "100000", "--days", "7",
                   "--qualification", "3", "--replication", "2", "--holdout", "2",
                   cwd=repo, env=env, timeout=28800)
        result["setup"] = {"returncode": setup.returncode,
                           "tail": (setup.stdout + setup.stderr)[-6000:]}
        if setup.returncode != 0:
            raise RuntimeError("owner ceremony/calibration failed:\n" + result["setup"]["tail"])

        port = free_port(); endpoint = f"http://127.0.0.1:{port}/chat/completions"
        preflight = py(os.path.join(repo, "run_observatory.py"), "--preflight",
                       "--run-id", RUN_ID, "--endpoint", endpoint,
                       "--model", MODEL, "--backend", "subprocess",
                       "--runtime", runtime, "--canonical-repo", target,
                       "--cp1-evidence", cp1, "--prior-cp2", prior,
                       cwd=repo, env=env, timeout=1800)
        result["preflight"] = {"returncode": preflight.returncode,
                               "tail": (preflight.stdout + preflight.stderr)[-6000:]}
        if preflight.returncode != 0:
            raise RuntimeError("protocol preflight failed:\n" + result["preflight"]["tail"])
        preflight_report = parse_json_output(preflight.stdout)

        mock_script = os.path.join(
            repo, "executable-orchestrator", "tools", "mock_observatory_protocol_server.py")
        mock = subprocess.Popen([sys.executable, mock_script, "--port", str(port)],
                                cwd=repo, env=env, stdout=subprocess.DEVNULL,
                                stderr=subprocess.DEVNULL)
        if not wait_port(port):
            raise RuntimeError("complete protocol mock server did not start")
        launch_env = dict(env); launch_env["DEEPSEEK_API_KEY"] = "local-proof-not-a-real-key"
        owner_key = os.path.join(repo, "private-evaluator",
                                 "owner-held-secrets", "OWNER.key")
        launch = drive_launch(repo, target, cp1, prior, runtime,
                              owner_key, endpoint, launch_env)
        result["launch"] = launch
        if not launch["completed"] or not launch["crash_seen"]:
            raise RuntimeError("shared state-machine launch/crash-resume did not complete")
        verified = verify_protocol(repo, runtime, source_head, preflight_report, launch)
        ...
        guard_out = os.path.join(runtime, "proof", "terminal-condition-closure.json")
        guard = py(os.path.join(repo, "executable-orchestrator", "tools",
                                "prove_terminal_condition_closure.py"),
                   "--runtime", runtime, "--run-id", RUN_ID,
                   "--canonical-repo", target, "--cp1-evidence", cp1,
                   "--prior-cp2", prior, "--endpoint", endpoint,
                   "--out", guard_out, cwd=repo, env=launch_env, timeout=3600)
        result["terminal_guard"] = {"returncode": guard.returncode,
                                    "tail": (guard.stdout + guard.stderr)[-6000:]}
        if guard.returncode != 0 or read_json(guard_out).get("status") != "PASS":
            raise RuntimeError("terminal condition negative proof failed:\n"
        ...
        output = os.path.join(ROOT, "proof", "complete-observatory-protocol-e2e.json")
