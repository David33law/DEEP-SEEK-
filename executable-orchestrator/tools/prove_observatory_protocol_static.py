...
partial proof entrypoints are only compatibility shims to the v4 E2E seat.
"""
import ast
import importlib
import json
...
EXPECTED_DIMENSIONS = {
    ...
    "cross_model_consistency_survival", "external_model_dependence"}
EXPECTED_MODULES = [
    ...
    "lawmax21.observatory_setup_v3", "lawmax21.observatory_preflight_v4",
    "lawmax21.observatory_implementation_search_overlay",
    "lawmax21.observatory_distributed_overlay", "lawmax21.observatory_scale_overlay",
...
def main():
    result = {"proof": "observatory-protocol-static-v4", "provider_calls": 0,
              "status": "FAIL"}
    try:
        ...
        for relative in files:
            ...
            if relative.endswith(".py"):
                ...
                compile(source, relative, "exec"); ast.parse(source, filename=relative)
                compiled.append(relative)
            elif relative.endswith(".json"):
                read_json(path); parsed.append(relative)
        imported = []
        for module in EXPECTED_MODULES:
            importlib.import_module(module); imported.append(module)
        missing_files = sorted(EXPECTED_FILES - set(files))
        if missing_files:
            raise RuntimeError("protocol census omitted: " + ", ".join(missing_files))
        if observatory_protocol.PROTOCOL_VERSION != "OBSERVATORY-OMEGA-RESEARCH-PROTOCOL-4":
        ...
        for section in observatory_protocol.PRODUCTION_WORKLOADS:
            ...
            if set(production) != set(proof):
                raise RuntimeError(section + ": proof/production workload keys differ")
            if not any(int(production[key]) > int(proof[key]) for key in production):
                raise RuntimeError(section + ": production workload is not stronger than proof")
        ...
        if len(ids) != len(set(ids)) or not EXPECTED_DIMENSIONS.issubset(set(ids)):
        ...
        source_ids = [row.get("source_id") for row in manifest.get("sources") or []]
        if len(source_ids) < 12 or len(source_ids) != len(set(source_ids)):
            raise RuntimeError("prior-art manifest is too small or has duplicate IDs")
        audit = _source("executable-orchestrator/lawmax21/observatory_audit.py")
        ordered = ["observatory_shared_corpus_hardening",
                   "observatory_evaluator_routing_hardening",
                   "observatory_scale_hardening",
                   "observatory_cross_model_workload_hardening",
                   "observatory_prior_art_hardening_overlay",
                   "base.install", "observatory_cross_model_overlay.install",
                   "observatory_phase_gate_hardening"]
        positions = [audit.index(token) for token in ordered]
        if positions != sorted(positions):
            raise RuntimeError("final audit overlay order is not the declared order")
        for relative in ("executable-orchestrator/tools/run_observatory_proof.py",
                         "executable-orchestrator/tools/prove_active_novelty_saturation.py"):
            text = _source(relative)
            if "prove_complete_observatory_protocol" not in text or len(text.splitlines()) > 15:
                raise RuntimeError(relative + " remains a duplicate proof implementation")
        bundle = observatory_protocol.protocol_bundle_sha256(ROOT)
        if len(bundle) != 64:
            raise RuntimeError("protocol bundle hash malformed")
        result.update({"status": "PASS", "protocol_version": observatory_protocol.PROTOCOL_VERSION,
                       "protocol_files": len(files), "python_files_compiled": len(compiled),
                       "json_files_parsed": len(parsed), "modules_imported": imported,
    ...
    output = os.path.join(ROOT, "proof", "observatory-protocol-static.json")
